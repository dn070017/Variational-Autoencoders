...
class ProMFCVAE(ProVLAE):
  def __init__(self, latent_dim, input_dims=(28, 28, 1), kernel_size=(3, 3), strides=(2, 2), num_components=10, num_facets=3, num_epoch_per_progress=10, fadein_iter=1500, gamma=0.5, prefix='promfcvae'):
    super().__init__(latent_dim, input_dims, kernel_size, strides, num_facets, num_epoch_per_progress, fadein_iter, gamma, prefix)
  ...
  def elbo(self, batch, **kwargs):
    ...
    for j in range(self.num_facets):
      ...
      kl_coefficient = beta
      if j < self.num_facets - self.progressive_step:
        kl_coefficient = self.gamma

      elbo += kl_coefficient * tf.reduce_mean(tf.reduce_sum(py_z_logpz_y, axis=-1))
      elbo += kl_coefficient * tf.reduce_mean(tf.reduce_sum(py_z_logpy, axis=-1))
      elbo += kl_coefficient * tf.reduce_mean(tf.reduce_sum(-py_z_logpy_z, axis=-1))
      elbo += kl_coefficient * tf.reduce_mean(-logqz_x)
    
    return tf.reduce_mean(elbo), tf.reduce_mean(logpx_z), tf.reduce_mean(logpx_z - elbo)
